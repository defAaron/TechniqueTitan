...
- Early webcam MediaPipe prototypes (since removed) established that landmark
  extraction over an OpenCV camera feed is feasible on the target machine.
- `src/technique_titan/` — the installable package with detection, geometry,
  scoring, coaching, and batch processing modules.
- Product surfaces: `api/` (FastAPI), `web/` (React), and `app.py` (Streamlit).
...
- Landmark extraction over a live camera feed is implemented via MediaPipe Hands + OpenCV in `src/technique_titan/detection/hand_detector.py`, with live feedback in Streamlit (`app.py`) and the React + API product stack (`web/`, `api/`).
